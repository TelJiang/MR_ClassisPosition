...
namespace mv
{
	...
	bool lineFit_X(const vector<cv::Point2f> &points, double &a, double &b, double &c, double &_x_Dxx);
	bool lineFit_Y(const vector<cv::Point2f> &points, double &a, double &b, double &c, double &_y_Dxx);

	const int simpleCameraWidth = 490;
	const int simpleCameraHeight = 430;
