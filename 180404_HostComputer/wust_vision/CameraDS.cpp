...
bool CCameraDS::OpenCamera(int nCamID, bool bDisplayProperties, int nWidth, int nHeight)
{
	...
	//	long Min, Max, Step, Default, Flags, Val;
	//	Val = (long)-50;
	//	hr = pProcAmp_1->GetRange(VideoProcAmp_Brightness, &Min, &Max, &Step,
	//		&Default, &Flags);
	//	std::cout << "亮度Default:" << Default << std::endl;
	//	if (hr == S_OK)
	//	{
	//		// Get the current value.
	//		hr = pProcAmp_1->Set(VideoProcAmp_Brightness, Val, VideoProcAmp_Flags_Auto);
	//		if (FAILED(hr))
	//		{
	//			std::cout << "set WhiteBalance failed" << std::endl;
	//		}
	...
	m_pCB->lWidth = m_nWidth;
	m_pCB->lHeight = m_nHeight;
...
IplImage* CCameraDS::QueryFrame()
{
	...
	//光圈
	//IAMVideoProcAmp *pProcAmp_1 = 0;
	//hr = m_pDeviceFilter->QueryInterface(IID_IAMCameraControl, (void**)&pProcAmp_1);
	//if (FAILED(hr))
	//{
	//	std::cout << "fail" << std::endl;
	//}
	...
	//	long Min, Max, Step, Default, Flags, Val;
	//	Val = (long)-6;
	//	hr = pProcAmp_1->GetRange(CameraControl_Exposure, &Min, &Max, &Step,
	//		&Default, &Flags);
	//	std::cout << Min << "	" << Max << std::endl;
	...
	//	{
	//		//Get the current value.
	//		hr = pProcAmp_1->Set(CameraControl_Exposure, Val, VideoProcAmp_Flags_Manual);
	//		if (FAILED(hr))
	//		{
	//			std::cout << "set Exposure failed" << std::endl;				
	//		}
	...
	while (!m_pCB->bReadyGO) {
		Sleep(2); // 切到下一个时间片，确保复制（原子）操作完成  
	}
	m_pCB->bGrabVideo = TRUE;
	if (m_pCB->bReadyGO == TRUE && m_pFrame != NULL)
	{
